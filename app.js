(function () {
  ...
  const levels = [
    ...
    { level: 2, identity: '成长店主', condition: '店铺客户 ≥ 3', relation: '全部满足', commission: '方案1.0', benefitIds: ['B01'], benefits: '鞋服业务收益', enabled: true, targets: [3, 0, 0], upgradeMode: '自动升级' },
    { level: 3, identity: '轻享店主', condition: '店铺客户5｜店铺收益¥2,000｜团队店主6', relation: '全部满足', commission: '方案1.1', benefitIds: ['B01', 'B03'], benefits: '深度线上运营培训', enabled: true, targets: [5, 2000, 6], upgradeMode: '自动升级' },
    { level: 4, identity: '轻享店主', condition: '店铺客户12｜店铺收益¥5,000｜团队店主12', relation: '全部满足', commission: '方案1.2', benefitIds: ['B01', 'B03'], benefits: '本级无新增权益', enabled: true, targets: [12, 5000, 12], upgradeMode: '自动升级' },
    { level: 5, identity: '轻享店主', condition: '店铺客户20｜店铺收益¥10,000｜团队店主20', relation: '全部满足', commission: '方案1.2', benefitIds: ['B01', 'B02', 'B03'], benefits: '手机回收品类', enabled: true, targets: [20, 10000, 20], upgradeMode: '自动升级' },
    { level: 6, identity: '星享店主', condition: '店铺客户30｜店铺收益¥18,000｜团队店主35', relation: '全部满足', commission: '方案1.5', benefitIds: ['B01', 'B02', 'B03'], benefits: '本级无新增权益', enabled: true, targets: [30, 18000, 35], upgradeMode: '自动升级' },
    { level: 7, identity: '星享店主', condition: '店铺客户40｜店铺收益¥28,000｜团队店主55', relation: '全部满足', commission: '方案1.8', benefitIds: ['B01', 'B02', 'B03'], benefits: '本级无新增权益', enabled: true, targets: [40, 28000, 55], upgradeMode: '自动升级' },
    { level: 8, identity: '星享店主', condition: '店铺客户50｜店铺收益¥40,000｜团队店主80', relation: '全部满足', commission: '方案2.0', benefitIds: ['B01', 'B02', 'B03', 'B04'], benefits: '每月30条成片素材', enabled: true, targets: [50, 40000, 80], upgradeMode: '自动升级' },
    { level: 9, identity: '超级店主', condition: '店铺客户65｜店铺收益¥65,000｜团队店主120', relation: '全部满足', commission: '方案2.1', benefitIds: ['B01', 'B02', 'B03', 'B04'], benefits: '本级无新增权益', enabled: true, targets: [65, 65000, 120], upgradeMode: '自动升级' },
    { level: 10, identity: '超级店主', condition: '店铺客户90｜店铺收益¥100,000｜团队店主180', relation: '全部满足', commission: '方案2.2', benefitIds: ['B01', 'B02', 'B03', 'B04'], benefits: '本级无新增权益', enabled: true, targets: [90, 100000, 180], upgradeMode: '自动升级' },
    { level: 11, identity: '超级店主', condition: '店铺客户120｜店铺收益¥160,000｜团队店主260', relation: '全部满足', commission: '方案2.3', benefitIds: ['B01', 'B02', 'B03', 'B04'], benefits: '专属运营支持', enabled: true, targets: [120, 160000, 260], upgradeMode: '自动升级' },
  ...
  function renderBenefits() {
    ...
    const groupedRows = categories.map((category) => {
      ...
      return `<div class="benefit-category-row ${benefitCategoryTone(category)}"><span>${category}</span><small>${categoryRows.length} 项 · 类别顺序固定，仅可调整组内展示顺序</small></div>${categoryRows.map((row) => {
        ...
        return `<div class="table-row benefit-table" draggable="true" data-benefit-row="${row.id}" data-benefit-drag="${row.id}" data-benefit-category="${row.category}"><button type="button" class="drag-handle" aria-label="拖拽调整${row.name}排序" title="仅可在${row.category}内拖拽排序">⋮⋮</button><span class="benefit-name"><i>${row.icon || '权'}</i><span><strong>${row.name}</strong><small>${row.id} · ${row.shortDescription || row.description}</small></span></span><span><strong>${row.order}</strong><small>展示排序</small></span><span class="reference-summary">${referenced ? `<strong>${references.slice(0, 4).join('、')}${references.length > 4 ? '等' : ''}</strong><small>当前线上版本引用</small>` : '<strong>未引用</strong><small>可停用或删除</small>'}</span>${pill(row.status)}<div class="row-actions"><button data-view-benefit="${row.id}">查看</button><button data-edit-benefit="${row.id}">编辑</button><button data-toggle-benefit="${row.id}" ${referenced && row.status === '生效中' ? 'class="locked-action" title="当前线上版本引用中"' : ''}>${row.status === '生效中' ? '停用' : '恢复'}</button><button class="danger-link ${referenced ? 'locked-action' : ''}" data-delete-benefit="${row.id}" ${referenced ? 'title="当前线上版本引用中"' : ''}>删除</button></div></div>`;
      }).join('')}`;
    }).join('');
    return `${pageHead('<button class="primary-button" data-new-benefit>新建权益</button>')}<div class="filter-bar"><input type="search" data-table-search placeholder="搜索权益名称或类别" value="${state.search}"><select><option>全部类别</option><option>收益类</option><option>运营类</option><option>鉴定类</option></select><span class="filter-meta">共 ${rows.length} 项权益</span></div><section class="data-table benefit-library"><div class="table-row head benefit-table"><span></span><span>权益名称</span><span>展示排序</span><span>当前引用</span><span>状态</span><span>操作</span></div>${groupedRows}</section><p class="sort-note">排序说明：拖拽仅会改变同一类别中的展示顺序；被当前线上版本引用的权益不可停用或删除。</p>`;
  }
  ...
  function openBenefitDrawer(mode, id) {
    ...
    const body = `<section class="form-section"><div class="form-section-title"><strong>前台展示信息</strong><span>${readOnly ? '只读查看' : '作为下一版本的待发布修改'}</span></div><div class="field"><label>权益名称</label><input id="benefit-name" value="${item.name}" ${disabled} placeholder="请输入权益名称"></div><div class="field-row equal"><div class="field"><label>权益类别</label><select id="benefit-category" ${disabled}>${['收益类','运营类','鉴定类'].map((value) => `<option ${item.category === value ? 'selected' : ''}>${value}</option>`).join('')}</select></div><div class="field"><label>展示排序</label><input id="benefit-order" type="number" min="1" step="10" value="${item.order || 10}" ${disabled}></div></div><div class="field"><label>短说明</label><input id="benefit-short-description" value="${item.shortDescription || item.description || ''}" ${disabled} placeholder="用于权益卡片摘要"></div><div class="field"><label>详细说明</label><textarea id="benefit-detail-description" ${disabled} placeholder="用于权益详情和规则说明">${item.detailDescription || item.description || ''}</textarea></div></section><section class="form-section"><div class="form-section-title"><strong>业务规则</strong></div><div class="field"><label>当前状态</label><select id="benefit-status" ${disabled}><option ${item.status === '生效中' ? 'selected' : ''}>生效中</option><option ${item.status === '已暂停' ? 'selected' : ''}>已暂停</option></select></div></section>`;
